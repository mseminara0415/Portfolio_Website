...
def lambda_handler(event, context):
    ...
    for satellite_id in returned_satellites:
        # Get satellite data
        satellite_data = download_satellite_data(n2yo_secrets=n2yo_secrets, norad_id=satellite_id, is_tle=False)
    
        # Get Json schema to validate against
        with open('2y2o_data_validation_schema_position.json') as file:
